...
computation of chunk weights:
\begin{enumerate}
\item Throw out chunks with bad DSST or containing no Doppler
...
\item Computes the RV zero point offsets for all chunks and adjust all
  ...
  for each chunk by doing, say, for the i$^{\rm th}$ chunk ${\rm ch = i}$:
  \beq
  v_{\rm ob, ch=i} -= \sum_{\rm ob=1}^{\rm ob=N_{obs}}{v_{\rm ob,
      ch=i}}/N_{\rm obs}
  \eeq
  where $v_{\rm obs,ch}$ means the reported RV for a chunk with
  index ch in observation labeled as obs, and there are $N_{\rm obs}$
  observations total. This is basically requiring that the mean RV
  ...
  star).
\item Chunk weights are computed as the inverse of the estimated RV
